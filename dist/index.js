...
exports.makeUtils = exports.custom = void 0;
 ...
 */
const makeUtils = (build, keyword, table, parentTable, allowInherit, relevantRelation) => {
    const Keyword = keyword[0].toUpperCase() + keyword.slice(1);
    const { inflection, getTypeByName, options: { [`pg${Keyword}ColumnName`]: columnNameToCheck = `is_${keyword}`, 
    // If true inverts the omitting logic (e.g. true for `is_published` or
    // `published_at`; false for `is_archived` or `archived_at`).
    [`pg${Keyword}ColumnImpliesVisible`]: invert = false, [`pg${Keyword}Relations`]: applyToRelations = false, [`pg${Keyword}Expression`]: expression = null, [`pg${Keyword}Tables`]: rawTables = null, }, } = build;
    const sql = build.pgSql;
    const introspectionResultsByKind = build.pgIntrospectionResultsByKind;
    const OptionType = getTypeByName(`Include${Keyword}Option`);
    if (expression && build.options[`pg${Keyword}ColumnName`]) {
        throw new Error(`'pg${Keyword}Expression' cannot be combined with 'pg${Keyword}ColumnName'`);
    }
    if (expression && build.options[`pg${Keyword}ColumnImpliesVisible`]) {
        throw new Error(`'pg${Keyword}Expression' cannot be combined with 'pg${Keyword}ColumnImpliesVisible'`);
    }
    if (expression && !build.options[`pg${Keyword}Tables`]) {
        throw new Error(`'pg${Keyword}Expression' requires 'pg${Keyword}Tables' to be set to a list of the tables to which this expression can apply.`);
    }
    ...
    const tables = rawTables
        ...
        : null;
    const getRelevantColumn = (tableToCheck) => tableToCheck
        ? introspectionResultsByKind.attribute.find((attr) => attr.classId === tableToCheck.id && attr.name === columnNameToCheck)
        : null;
    const _tableIsAllowed = (table) => table != null &&
        (tables == null ||
            tables.some((t) => table.namespaceName === t[0] && table.name === t[1]));
    const appliesToTable = (table) => _tableIsAllowed(table) && (expression || getRelevantColumn(table));
    if (relevantRelation) {
        if (!applyToRelations && !relevantRelation.tags[`${keyword}Relation`]) {
            return null;
        }
        if (!relevantRelation.foreignClass ||
    ...
    const argumentName = inflection[`include${Keyword}Argument`](table, relevantRelation);
    ...
    const pgParentRelevantColumnIsBoolean = parentTableRelevantColumn &&
    ...
    const rawLocalDetails = expression
        ? appliesToTable(relevantClass)
            ? {
                expression: (_sql, tableAlias) => sql.fragment `(${expression(sql, tableAlias)})`,
        ...
        : relevantColumn
            ? {
                expression: (_sql, tableAlias) => sql.fragment `${tableAlias}.${sql.identifier(relevantColumn.name)}`,
    ...
    const parentDetails = appliesToTable(parentTable)
        ? expression
            ...
            : parentTableRelevantColumn
                ? {
                    expression: (_sql, tableAlias) => sql.fragment `${tableAlias}.${sql.identifier(parentTableRelevantColumn.name)}`,
    ...
    function addWhereClause(queryBuilder, fieldArgs) {
        ...
        let fragment = null;
        const myAlias = relevantClass !== table
        ...
        const relevantSettingIfNotInherit = relevantSetting !== "INHERIT"
            ? relevantSetting
        ...
        if (capableOfInherit &&
            ...
            fragment = sql.fragment `(${parentDetails.expression(sql, sqlParentTableAlias)} is ${parentDetails.invisibleFragment} or ${localDetails.expression(sql, myAlias)} is ${localDetails.visibleFragment})`;
        }
        else if (relevantSettingIfNotInherit === "NO") {
            fragment = sql.fragment `${localDetails.expression(sql, myAlias)} is ${localDetails.visibleFragment}`;
        }
        else if (relevantSettingIfNotInherit === "EXCLUSIVELY") {
            fragment = sql.fragment `${localDetails.expression(sql, myAlias)} is ${localDetails.invisibleFragment}`;
        }
        if (fragment) {
            if (relevantRelation && relevantClass !== table) {
                const localAlias = queryBuilder.getTableAlias();
                const relationConditions = relevantRelation.keyAttributes.map((attr, i) => {
                    const otherAttr = relevantRelation.foreignKeyAttributes[i];
                    return sql.fragment `${localAlias}.${sql.identifier(attr.name)} = ${myAlias}.${sql.identifier(otherAttr.name)}`;
                });
                const subquery = sql.fragment `exists (select 1 from ${sql.identifier(relevantClass.namespaceName, relevantClass.name)} as ${myAlias} where (${sql.join(relationConditions, ") and (")}) and (${fragment}))`;
...
const generator = (keyword = "archived") => {
    ...
    */
    const AddInflectorsPlugin = (builder) => {
        builder.hook("inflection", (inflection, build) => {
            return build.extend(inflection, {
                [`include${Keyword}Argument`](table, relation) {
                    const relationPart = relation
                        ? inflection.singleRelationByKeys(relation.keyAttributes, relation.foreignClass, table, relation)
                        : null;
    ...
    const AddToEnumPlugin = (builder) => {
        ...
        builder.hook("build", (build) => {
            const { graphql: { GraphQLEnumType }, } = build;
            const defaultValue = build.options[`pg${Keyword}Default`] || "NO";
            build.newWithHooks(GraphQLEnumType, {
                ...
                values: {
                    ...
                    INHERIT: {
                        description: `If there is a parent GraphQL record and it is ${keyword} then this is equivalent to YES, in all other cases this is equivalent to ${defaultValue === "INHERIT" ? "NO" : defaultValue}.`,
                        value: "INHERIT",
                    },
                },
            }, {
    ...
    const PgOmitInnerPlugin = (builder) => {
        builder.hook("GraphQLObjectType:fields:field:args", (args, build, context) => {
            const { extend } = build;
            const { scope: { isPgFieldConnection, isPgFieldSimpleCollection, isPgBackwardRelationField, pgFieldIntrospection, pgIntrospection: parentTable, [`include${Keyword}`]: includeArchived, }, addArgDataGenerator, Self, field, } = context;
            const defaultValue = build.options[`pg${Keyword}Default`] || "NO";
            const defaultInherit = build.options[`pg${Keyword}DefaultInherit`] !== false;
            const table = pgFieldIntrospection;
            ...
            const allUtils = selfAndConstraints.map((relation) => makeUtils(build, keyword, table, parentTable, allowInherit, relation));
            if (!allUtils) {
                return args;
            }
            return allUtils.reduce((args, utils) => {
                ...
                const { addWhereClause, OptionType, capableOfInherit, argumentName, } = utils;
                ...
                return extend(args, {
                    [argumentName]: {
                        description: `Indicates whether ${keyword} items should be included in the results or not.`,
                        type: OptionType,
                        defaultValue: capableOfInherit && defaultInherit ? "INHERIT" : defaultValue,
                    },
                }, `Adding ${argumentName} argument to connection field '${field.name}' of '${Self.name}'`);
            }, args);
        });
    };
    const Plugin = graphile_utils_1.makePluginByCombiningPlugins(AddInflectorsPlugin, AddToEnumPlugin, PgOmitInnerPlugin);
    Plugin.displayName = `PgOmit${Keyword}Plugin`;
    return Plugin;
};
exports.custom = generator;
